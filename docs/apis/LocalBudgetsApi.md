# EdFi.OdsApi.Sdk.v72.Apis.All.LocalBudgetsApi
...
namespace Example
{
    public class GetLocalBudgetsExample
    {
        public static void Main()
        {
            ...
            {
                // Retrieves specific resources using the resource's property values (using the \"Get\" pattern).
                List<EdFiLocalBudget> result = apiInstance.GetLocalBudgets(offset, limit, minChangeVersion, maxChangeVersion, totalCount, asOfDate, accountIdentifier, educationOrganizationId, fiscalYear, financialCollectionDescriptor, amount, id, useSnapshot);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
...
{
    // Retrieves specific resources using the resource's property values (using the \"Get\" pattern).
    ApiResponse<List<EdFiLocalBudget>> response = apiInstance.GetLocalBudgetsWithHttpInfo(offset, limit, minChangeVersion, maxChangeVersion, totalCount, asOfDate, accountIdentifier, educationOrganizationId, fiscalYear, financialCollectionDescriptor, amount, id, useSnapshot);
    Debug.Write("Status Code: " + response.StatusCode);
    Debug.Write("Response Headers: " + response.Headers);
    Debug.Write("Response Body: " + response.Data);
}

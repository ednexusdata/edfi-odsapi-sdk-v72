# EdFi.OdsApi.Sdk.v72.Apis.All.StudentTitleIPartAProgramAssociationsApi
...
namespace Example
{
    public class GetStudentTitleIPartAProgramAssociationsExample
    {
        public static void Main()
        {
            ...
            {
                // Retrieves specific resources using the resource's property values (using the \"Get\" pattern).
                List<EdFiStudentTitleIPartAProgramAssociation> result = apiInstance.GetStudentTitleIPartAProgramAssociations(offset, limit, minChangeVersion, maxChangeVersion, totalCount, beginDate, educationOrganizationId, programEducationOrganizationId, programName, programTypeDescriptor, studentUniqueId, titleIPartAParticipantDescriptor, id, useSnapshot);
                Debug.WriteLine(result);
            }
            catch (ApiException  e)
            {
